...
const MAX_FUZZY_QUERY_CHARS: usize = 72;
const FUZZY_MIN_QUERY_CHARS: usize = 3;
const FUZZY_FILL_RATIO_GATE: f64 = 0.42;
const SEMANTIC_TABLE_NAME: &str = "semantic_hits_v1";
const SEMANTIC_META_FILE_NAME: &str = "semantic-index-meta-v1.json";
const SEMANTIC_DB_DIR_NAME: &str = "semantic-lancedb";
const SEMANTIC_MAX_DOCUMENTS: usize = 220_000;
const SEMANTIC_EMBED_BATCH: usize = 24;
const SEMANTIC_MAX_TOKENS: usize = 192;
...
fn database_path(app: &AppHandle) -> CommandResult<PathBuf> {
    Ok(app_data_dir(app)?.join("blockfile-index-v1.sqlite3"))
}

fn semantic_db_dir(app: &AppHandle) -> CommandResult<PathBuf> {
    Ok(app_data_dir(app)?.join(SEMANTIC_DB_DIR_NAME))
}

fn semantic_meta_path(app: &AppHandle) -> CommandResult<PathBuf> {
    Ok(app_data_dir(app)?.join(SEMANTIC_META_FILE_NAME))
}
...
fn should_run_fuzzy_fallback(
    ...
    if query_len > MAX_FUZZY_QUERY_CHARS {
        return false;
    }
...
fn load_semantic_runtime(app: &AppHandle) -> CommandResult<&'static Mutex<SemanticRuntime>> {
    SEMANTIC_RUNTIME.get_or_try_init(|| {
...
fn semantic_root_fingerprint_ms(connection: &Connection) -> CommandResult<i64> {
    connection
        .query_row(
...
fn load_semantic_candidates(
    ...
    let max_documents_i64 = i64::try_from(max_documents).unwrap_or(0);
    let heading_limit = (max_documents_i64.saturating_mul(58) / 100).clamp(240, max_documents_i64);
    let author_limit = (max_documents_i64.saturating_mul(22) / 100).clamp(120, max_documents_i64);
    let file_limit = (max_documents_i64.saturating_mul(20) / 100).clamp(120, max_documents_i64);
...

    {
        let mut statement = connection
            ...
            .map_err(|error| format!("Could not prepare semantic heading candidates query: {error}"))?;

        let rows = statement
            .query_map(params![heading_limit], |row| {
                Ok((
                    row.get::<_, i64>(0)?,
                    row.get::<_, i64>(1)?,
                    row.get::<_, String>(2)?,
            ...
            .map_err(|error| format!("Could not run semantic heading candidates query: {error}"))?;

        for row in rows {
            if candidates.len() >= max_documents {
                break;
            }
            let (root_id, file_id, relative_path, absolute_path, heading_level, heading_text, heading_order) =
                row.map_err(|error| format!("Could not parse semantic heading candidate: {error}"))?;
            ...
            let semantic_text = semantic_embedding_text(&format!(
                "heading: {}\npath: {}\nfile: {}",
                heading_text, relative_path, file_name
            ));
...

    {
        let mut statement = connection
            ...
            .map_err(|error| format!("Could not prepare semantic author candidates query: {error}"))?;

        let rows = statement
            .query_map(params![author_limit], |row| {
                Ok((
                    row.get::<_, i64>(0)?,
                    row.get::<_, i64>(1)?,
                    row.get::<_, String>(2)?,
...

    {
        let mut statement = connection
            ...
            .map_err(|error| format!("Could not prepare semantic file candidates query: {error}"))?;

        let rows = statement
            .query_map(params![file_limit], |row| {
                Ok((
                    row.get::<_, i64>(0)?,
                    row.get::<_, i64>(1)?,
                    row.get::<_, String>(2)?,
            ...
            .map_err(|error| format!("Could not run semantic file candidates query: {error}"))?;
        ...
        for row in rows {
            ...
            let (root_id, file_id, relative_path, absolute_path) =
                row.map_err(|error| format!("Could not parse semantic file candidate: {error}"))?;
            let file_name = file_name_from_relative(&relative_path);
            let semantic_text = semantic_embedding_text(&format!(
                "file: {}\npath: {}",
...
fn encode_semantic_batch(
    ...
    Ok((input_ids, attention_mask, token_type_ids, batch_size, seq_len))
}

fn embed_semantic_texts(app: &AppHandle, texts: &[String]) -> CommandResult<Vec<Vec<f32>>> {
    if texts.is_empty() {
        return Ok(Vec::new());
    }
    ...
    let outputs = runtime
        ...
        .run(
            ort::inputs! {
                "input_ids" => OrtTensor::from_array((shape.clone(), input_ids.clone())).map_err(|error| format!("Could not create semantic input_ids tensor: {error}"))?,
                "attention_mask" => OrtTensor::from_array((shape.clone(), attention_mask.clone())).map_err(|error| format!("Could not create semantic attention_mask tensor: {error}"))?,
                "token_type_ids" => OrtTensor::from_array((shape.clone(), token_type_ids.clone())).map_err(|error| format!("Could not create semantic token_type_ids tensor: {error}"))?
            },
        )
        .or_else(|_| {
            runtime.session.run(
                ort::inputs! {
                    "input_ids" => OrtTensor::from_array((shape.clone(), input_ids)).map_err(|error| format!("Could not create semantic fallback input_ids tensor: {error}"))?,
                    "attention_mask" => OrtTensor::from_array((shape, attention_mask)).map_err(|error| format!("Could not create semantic fallback attention_mask tensor: {error}"))?
                },
            )
        })
        .map_err(|error| format!("Semantic model inference failed: {error}"))?;

    let output = if outputs.contains_key(runtime.output_name.as_str()) {
        &outputs[runtime.output_name.as_str()]
    } else {
        &outputs[0]
    };
    let output = output
...
fn semantic_record_batch(
    ...
    let semantic_ids = Int64Array::from_iter_values(candidates.iter().map(|candidate| candidate.semantic_id));
    let root_ids = Int64Array::from_iter_values(candidates.iter().map(|candidate| candidate.root_id));
    let kinds = StringArray::from_iter_values(candidates.iter().map(|candidate| candidate.kind.as_str()));
    let file_ids = Int64Array::from_iter_values(candidates.iter().map(|candidate| candidate.file_id));
    let file_names =
        StringArray::from_iter_values(candidates.iter().map(|candidate| candidate.file_name.as_str()));
    let relative_paths =
        StringArray::from_iter_values(candidates.iter().map(|candidate| candidate.relative_path.as_str()));
    let absolute_paths =
        StringArray::from_iter_values(candidates.iter().map(|candidate| candidate.absolute_path.as_str()));
    let heading_levels =
        Int64Array::from(candidates.iter().map(|candidate| candidate.heading_level).collect::<Vec<_>>());
    ...
    let heading_orders =
        Int64Array::from(candidates.iter().map(|candidate| candidate.heading_order).collect::<Vec<_>>());
...
async fn rebuild_semantic_index(app: AppHandle, force: bool) -> CommandResult<()> {
    ...
    for chunk in candidates.chunks(SEMANTIC_EMBED_BATCH) {
        ...
        let batch = semantic_record_batch(
            schema.clone().ok_or_else(|| "Semantic schema was not initialized".to_string())?,
            chunk,
            &embeddings,
            embedding_dim,
        )?;
...
fn trigger_semantic_rebuild(app: AppHandle, force: bool) {
...
fn semantic_hits_from_batches(batches: &[RecordBatch], limit: usize) -> CommandResult<Vec<SearchHit>> {
    ...
    for batch in batches {
        ...
        for row_index in 0..batch.num_rows() {
            ...
            let heading_level = heading_level_col.and_then(|column| {
            ...
            let heading_order = heading_order_col.and_then(|column| {
                (!column.is_null(row_index)).then_some(column.value(row_index))
            });
            ...
            let distance = distance_f32
                .and_then(|column| (!column.is_null(row_index)).then_some(f64::from(column.value(row_index))))
                .unwrap_or(1.0);
            hits.push(SearchHit {
...
fn index_root(app: AppHandle, path: String) -> CommandResult<IndexStats> {
    ...
    // Rebuild semantic vectors asynchronously after lexical index updates complete.
    trigger_semantic_rebuild(app.clone(), true);
...
#[tauri::command]
async fn search_index_semantic(
    app: AppHandle,
    query: String,
    root_path: Option<String>,
    limit: Option<usize>,
    ...
    let capped_query = query
        ...
        .collect::<String>();
    let cleaned_query = capped_query.trim();
    if cleaned_query.chars().count() < SEMANTIC_MIN_QUERY_CHARS {
        return Ok(Vec::new());
    }
    ...
    trigger_semantic_rebuild(app.clone(), false);
    let semantic_limit = limit.unwrap_or(40).clamp(8, 120);
    semantic_search(&app, cleaned_query, requested_root_id, semantic_limit).await
}
